...
.. Place the _README_ file in the directory. The contents are in the
appendix <<Listing of README>>.

.. Copy the default _gromet.service_, and _gromet.yml_ files into the
directory. The contents are in the appendices
<<Listing of gromet.service>> and <<Listing of gromet.yml>>,
respectively.
...
.. Change the _listen_address_ as needed. Typically, `127.0.0.1:50001`
would be used to serve met. data to the local host. To server data to
the network, use the host alias for this machine from _/etc/hosts_ in place
of `127.0.0.1`.
...
[Service]
ExecStart=/usr2/st/gromet/gromet
Restart=always
User=oper
Group=rtx
...
= Listing of gromet.yml

....
listen_address: 127.0.0.1:50001
